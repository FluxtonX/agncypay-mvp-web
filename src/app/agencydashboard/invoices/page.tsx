...
export default function InvoicesQueuePage() {
  ...
  const expectedTotal = invoices.reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
  const awaitingBrandTotal = invoices.filter(i => i.status === "awaiting_approval").reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
  const settledTotal = invoices.filter(i => i.status === "settled" || i.status === "talent_disbursed").reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);

  const talentPoolTotal = invoices.reduce((acc, curr) => {
    const talentSplit = curr.splitPool.splits.find(s => s.role === "Talent");
    return acc + (talentSplit ? (Number(talentSplit.amount) || 0) : 0);
  }, 0);
  const pendingPayoutTotal = invoices.filter(i => i.status !== "talent_disbursed").reduce((acc, curr) => {
    const talentSplit = curr.splitPool.splits.find(s => s.role === "Talent");
    return acc + (talentSplit ? (Number(talentSplit.amount) || 0) : 0);
  }, 0);
  const disbursedTotal = invoices.filter(i => i.status === "talent_disbursed").reduce((acc, curr) => {
    const talentSplit = curr.splitPool.splits.find(s => s.role === "Talent");
    return acc + (talentSplit ? (Number(talentSplit.amount) || 0) : 0);
  }, 0);
    ...
    <AppShell>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-600 font-semibold">Invoices & Settlement Queue</span>
        </div>

        {/* Header Title & Action */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Payments & Invoicing</h1>
            <p className="text-xs text-slate-500 mt-1">Manage brand invoice receivables and talent payables in one real-time ledger.</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              className="h-10 px-5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs shrink-0 bg-slate-900 text-white hover:bg-slate-800"
            >
        ...
        {state.user && state.user.kybStatus !== "approved" && (
          <div className="rounded-2xl border border-amber-300 bg-amber-50/90 text-amber-950 p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-2xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-amber-900">Business Verification Required (KYB)</p>
                <p className="text-[11px] text-amber-800 mt-0.5">Complete business verification to unlock live ACH/Wire deposit bank accounts and automatic talent splitting.</p>
              </div>
            </div>
            <button
              onClick={() => router.push("/onboarding/business-setup")}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all cursor-pointer shrink-0 shadow-xs bg-amber-600 hover:bg-amber-700 text-white"
            >
        ...
        {/* Main Tabs (Receivables vs Payables) */}
          ...
          {activeMainTab === "receivables" ? (
            <>
              <MetricCard
                ...
                value={expectedTotal}
                isCurrency={true}
                deltaPeriod={`${invoices.length} total invoice(s)`}
                icon={<Landmark className="h-4 w-4 text-slate-500" />}
              />
              <MetricCard
                title="Awaiting Settlement"
                value={awaitingBrandTotal}
                isCurrency={true}
                deltaPeriod={`${invoices.filter(i => i.status === "awaiting_approval").length} pending approval`}
            ...
            <>
              <MetricCard
                ...
                isCurrency={true}
                deltaPeriod="Contracted allocations"
              ...
              <MetricCard
                ...
                isCurrency={true}
                deltaPeriod="Awaiting disbursement"
                icon={<Receipt className="h-4 w-4 text-amber-500" />}
              />
              <MetricCard
                title="Total Disbursed"
                value={disbursedTotal}
                isCurrency={true}
                deltaPeriod="Paid out to talent"
                icon={<ShieldCheck className="h-4 w-4 text-emerald-500" />}
              />
            </>
          )}
              ...
              [
                ...
                >
                  ...
                  {tab.id === "awaiting_approval" && (
                    <span className={`ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold ${activeReceivableFilter === tab.id ? "bg-white/20 text-white" : "bg-slate-100 text-slate-700"}`}>
              ...
              [
                ...
                >
                  ...
                  {tab.id === "pending_payout" && (
                    <span className={`ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold ${activePayableFilter === tab.id ? "bg-white/20 text-white" : "bg-slate-100 text-slate-700"}`}>
                        ...
                        {activeMainTab === "receivables" ? (
                          <>
                            <td className="p-4">
                              ...
                              <p className="text-[11px] text-slate-500 mt-0.5">{inv.campaignName}</p>
                            </td>
                            <td className="p-4 text-right font-semibold text-slate-900 text-sm">
                              <Money amount={Number(inv.amount) || 0} size="sm" />
                            </td>
                          </>
                        ) : (
                          <>
                            <td className="p-4">
                              <p className="font-bold text-slate-900">{inv.splitPool.splits.find(s => s.role === "Talent")?.name || "Talent"}</p>
                              <p className="text-[11px] text-slate-500 mt-0.5">{inv.campaignName}</p>
                            </td>
                            <td className="p-4 text-right font-semibold text-slate-900 text-sm">
                              <Money amount={Number(inv.splitPool.splits.find(s => s.role === "Talent")?.amount) || 0} size="sm" />
                            </td>
                          </>
                        )}
                        <td className="p-4 text-center">
                          {activeMainTab === "receivables" ? (
                            <TransactionStatusBadge
                              status={inv.status === "awaiting_approval" ? "PENDING" : "COMPLETED"}
                              size="sm"
                            />
                          ) : (
                            <TransactionStatusBadge
                              status={inv.status === "talent_disbursed" ? "COMPLETED" : "PENDING"}
                              size="sm"
                            />
                          )}
                        </td>
                        <td className="p-4 text-right pr-6">
                          <ChevronRight className="h-4 w-4 inline-block text-slate-400 group-hover:text-slate-900 transition-transform group-hover:translate-x-0.5" />
                        </td>
                      </tr>
                    );
                  })}
