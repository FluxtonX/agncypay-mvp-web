...
export function InstantBankVerification() {
  ...
  // Check active Plaid connection on load and fetch link_token from backend API
  useEffect(() => {
  ...
  const onSuccess = useCallback(
    async (public_token: string, metadata: any) => {
      setPlaidState("connecting");
      try {
        const res: any = await apiExchangePlaidPublicToken(public_token);
        setPlaidState("connected");
        updateBankDetails({
          accountHolderName: state.businessSetup.legalName || "Business account",
          bankName: metadata.institution?.name || res.bankDetails?.bankName || "Plaid Connected Bank",
          routingNumber: res.bankDetails?.routingNumber || "Verified via Plaid",
          accountNumber: res.bankDetails?.accountNumber || "Verified via Plaid",
          statementUploaded: true,
          status: "approved",
        });
      } catch (error: any) {
